...
- 通过 `core.RegisterAgent("acp", factory)` 在 `init()` 注册；`cmd/cc-connect/plugin_agent_acp.go` + `Makefile` + `config/config.example.toml` 与现有 agent 插件一致。
...
## 5. 配置草案（`config/config.example.toml`）
...
5. 文档：`docs/` 简短用户说明 + `config/config.example.toml` 示例。
