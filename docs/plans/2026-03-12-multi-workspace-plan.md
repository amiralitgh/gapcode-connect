...
- Modify: `config/config.example.toml` (add multi-workspace example)
...
**Step 3: Add example to config/config.example.toml**
...
```bash
git add config/config.go config/config.example.toml
...
### Task 12: Update config/config.example.toml and verify build
...
git commit -m "docs: add multi-workspace example to config/config.example.toml"
```
