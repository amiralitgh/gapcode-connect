...
## GapCode TUI commands and work directory (`/cd`, `/pwd`)

The Telegram and Bale menus expose GapCode's native TUI command names. The
bridge handles the commands that can be represented safely in chat and passes
the remaining GapCode commands through to the configured GapCode session.

### Chat Commands

```
/cd                     Show the current work directory
...
- These filesystem and launch controls are privileged commands. You must set
  `admin_from` under `[[projects]]` in `config.toml` before they can be used.
...
- Restart GapCode Connect after updating `config.toml`.
- Directory changes apply to the next GapCode session in the current project.
- Relative paths are resolved from the current agent work directory.
- The legacy `/dir` command remains available for compatibility but is not
  published in the Telegram or Bale command menu. `/ls`, `/mkdir`, and
  `/launch_gapcode` are not bridge commands.
...
[#496](https://github.com/amiralitgh/gapcode-connect/issues/496)) and an
...
See [config/config.example.toml](../config/config.example.toml) for full examples.
...
Minimal config snippet (full version is in `config/config.example.toml` under
