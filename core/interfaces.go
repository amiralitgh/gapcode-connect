...
// It looks up the message in the same messages bundle and falls back to
// English. Returning the key itself on total miss matches (*I18n).T.
//
...
//
// If a Farsi key is missing for one of the four tool sections, that section
// silently falls back to English so the agent always receives valid
// instructions.
//
// Pass LangEnglish for the historical English-only behaviour, which
// AgentSystemPrompt() is a thin wrapper around.
//
// See Issue #1655 for the original feature request.
func AgentSystemPromptForLang(lang Language) string {
	prompt := agentSystemPromptHeader +
	...
	if lang == LangFarsi {
		prompt += "\n\n### زبان پاسخ\nبه فارسی پاسخ بده، مگر اینکه کاربر زبان دیگری بخواهد. نام فایل‌ها، کد، دستورهای شل و شناسه‌ها را تغییر نده."
	}
	return prompt
}
...
// AllSessionsProvider optionally exposes sessions across every working
// directory. Agents that cannot provide this view can keep using ListSessions,
// which remains the current-workspace view.
...
// SessionTitleSetter is an optional agent capability for persisting a native
// session title in the agent's own session index.
...
type BotCommandInfo struct {
	...
	Source      string // builtin, agent, config, or skill
}
