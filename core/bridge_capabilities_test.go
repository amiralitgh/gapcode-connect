...
func TestEngine_GetBridgePublishedCommands_IncludesGapCodeAndCustoms(t *testing.T) {
	...
	for _, cmd := range commands {
		...
		case "model":
			foundModel = true
			if cmd.Source != bridgeCommandSourceGapCode {
				t.Fatalf("model source = %q, want %q", cmd.Source, bridgeCommandSourceGapCode)
			}
		case "deploy":
			foundDeploy = true
	...
	if foundHelp {
		t.Fatal("cc-connect help command leaked into bridge catalog")
	}
	if !foundModel {
		t.Fatal("expected GapCode model command in bridge catalog")
	}
	if !foundDeploy {
