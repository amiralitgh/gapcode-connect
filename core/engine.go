...
func (e *Engine) SetAdminFrom(adminFrom string) {
	...
	if af == "" && !shellDisabled {
		slog.Warn("admin_from is not set — privileged commands (/shell, /cd, /pwd, /show, /dir, /restart, /upgrade) are blocked. "+
...
var builtinCommands = []struct {
	...
	{[]string{"fork"}, "fork"},
	{[]string{"init"}, "init"},
	{[]string{"compact", "compress"}, "compact"},
	{[]string{"plan"}, "plan"},
	{[]string{"goal"}, "goal"},
...
type gapCodeTUICommand struct {
	name        string
	description string
}

var gapCodeTUICommandSpecs = []gapCodeTUICommand{
	{"model", "Choose the model and reasoning effort"},
	{"fast", "1.5x speed, 2x usage"},
	{"ide", "Include current IDE context"},
	{"permissions", "Choose what GapCode is allowed to do"},
	{"keymap", "Remap TUI shortcuts"},
	{"vim", "Toggle Vim mode for the composer"},
	{"experimental", "Toggle experimental features"},
	{"approve", "Approve one retry of a recent auto-review denial"},
	{"memories", "Configure memory use and generation"},
	{"skills", "Use skills to improve GapCode"},
	...
	{"agents", "View and switch active agent sessions"},
	...
	{"logout", "Log out of GapCode"},
	{"exit", "Exit GapCode"},
	{"feedback", "Send logs to maintainers"},
	{"ps", "List background terminals"},
	{"stop", "Stop background terminals"},
	{"personality", "Choose a communication style"},
	{"subagents", "Switch between subagents"},
}

var gapCodeTUICommandNames = func() []string {
	names := make([]string, 0, len(gapCodeTUICommandSpecs))
	for _, command := range gapCodeTUICommandSpecs {
		names = append(names, command.name)
	}
	return names
}()

func gapCodeTUICommands() []BotCommandInfo {
	commands := make([]BotCommandInfo, 0, len(gapCodeTUICommandSpecs))
	for _, command := range gapCodeTUICommandSpecs {
...
var gapCodeTUICommandSet = func() map[string]bool {
	commands := make(map[string]bool, len(gapCodeTUICommandSpecs))
	for _, command := range gapCodeTUICommandSpecs {
...
var builtinMenuAliases = map[string][]string{
	"name": {"rename"},
}
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	case "ps":
		e.cmdPs(p, msg, args)
	default:
		// These are GapCode TUI commands. Commands already implemented by the
		// bridge are handled above; the rest are forwarded unchanged so an
		// app-server/native GapCode backend can interpret them. They must not
		// be reported as cc-connect commands.
		if gapCodeTUICommandSet[cmdID] {
			return false
		}
...
func (e *Engine) listAgentSessions(agent Agent, sessions *SessionManager, all bool) ([]AgentSessionInfo, error) {
	var (
		agentSessions []AgentSessionInfo
		err           error
	)
	if all {
		if provider, ok := agent.(AllSessionsProvider); ok {
			agentSessions, err = provider.ListAllSessions(e.ctx)
		} else {
...
func (e *Engine) cmdList(p Platform, msg *Message, args []string) {
	...
	listArgs := args
	listAll := len(args) > 0 && strings.EqualFold(strings.TrimSpace(args[0]), "all")
	if listAll {
		listArgs = args[1:]
	}

	if !supportsCards(p) {
		agentSessions, err := e.listAgentSessions(agent, sessions, listAll)
		if err != nil {
		...
		page := 1
		if len(listArgs) > 0 {
			if n, err := strconv.Atoi(listArgs[0]); err == nil && n > 0 {
				page = n
			}
		}
		if page > totalPages {
...
func (e *Engine) cmdSwitch(p Platform, msg *Message, args []string) {
	...
	listAll := len(args) > 1 && strings.EqualFold(strings.TrimSpace(args[0]), "all")
	queryArgs := args
	if listAll {
		queryArgs = args[1:]
	}
	...
	if query == "" {
		e.reply(p, msg.ReplyCtx, "Usage: /switch [all] <number | id_prefix | name>")
		return
	}
...
func (e *Engine) cmdPwd(p Platform, msg *Message) {
	agent, _, _, err := e.commandContext(p, msg)
	if err != nil {
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgWsResolutionError, err))
		return
	}
	e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgDirCurrent, e.commandWorkDir(agent, msg)))
}

// cmdResume mirrors GapCode's resume picker in chat: without an argument it
// lists saved sessions; with an argument it resumes the selected session.
func (e *Engine) cmdResume(p Platform, msg *Message, args []string) {
	if len(args) == 0 {
		if _, ok := p.(InlineButtonSender); ok {
			e.sendResumeScopePicker(p, msg)
			return
		}
		e.reply(p, msg.ReplyCtx, "Resume a saved chat:\n\n`/resume cwd` — sessions in the current directory\n`/resume all` — sessions in every directory")
		return
	}

	scope := strings.ToLower(strings.TrimSpace(args[0]))
	if scope == "cwd" || scope == "all" {
		if len(args) > 1 && strings.EqualFold(strings.TrimSpace(args[1]), "pick") {
			if len(args) < 3 {
				e.reply(p, msg.ReplyCtx, "Usage: /resume pick <cwd|all> <number>")
				return
			}
			e.cmdSwitch(p, msg, resumeSwitchArgs(scope, args[2:]))
			return
		}
		if len(args) > 1 && strings.EqualFold(strings.TrimSpace(args[1]), "page") {
			args = args[2:]
		} else {
			args = args[1:]
		}
		e.sendResumeSessionPicker(p, msg, scope, resumePage(args))
		return
	}
	if scope == "pick" {
		if len(args) < 3 {
		...
		scope = strings.ToLower(strings.TrimSpace(args[1]))
		if scope != "cwd" && scope != "all" {
			e.reply(p, msg.ReplyCtx, "Usage: /resume pick <cwd|all> <number>")
			return
		}
...
func resumePage(args []string) int {
	if len(args) == 0 {
		return 1
	}
...
func resumeSwitchArgs(scope string, args []string) []string {
	if scope == "all" {
...
func (e *Engine) sendResumeScopePicker(p Platform, msg *Message) {
	content := "Resume a saved chat:\nChoose where to look for sessions."
	buttons := [][]ButtonOption{{
		{Text: "Current directory", Data: "cmd:/resume cwd"},
		{Text: "All directories", Data: "cmd:/resume all"},
	}}
	e.replyWithButtons(p, msg.ReplyCtx, content, buttons)
}

func (e *Engine) sendResumeSessionPicker(p Platform, msg *Message, scope string, page int) {
	agent, sessions, _, err := e.commandContext(p, msg)
	if err != nil {
		e.reply(p, msg.ReplyCtx, e.i18n.Tf(MsgWsResolutionError, err))
		return
	...
	all := scope == "all"
	agentSessions, err := e.listAgentSessions(agent, sessions, all)
	if err != nil {
		e.reply(p, msg.ReplyCtx, fmt.Sprintf(e.i18n.T(MsgListError), err))
		return
	}
	...
	totalPages := (len(agentSessions) + listPageSize - 1) / listPageSize
	if page > totalPages {
		page = totalPages
	}
	start := (page - 1) * listPageSize
	end := start + listPageSize
	if end > len(agentSessions) {
		end = len(agentSessions)
	}

	scopeTitle := "Current directory"
	if all {
		scopeTitle = "All directories"
	}
	var body strings.Builder
	body.WriteString(fmt.Sprintf("Resume — %s (%d sessions)", scopeTitle, len(agentSessions)))
	if totalPages > 1 {
		body.WriteString(fmt.Sprintf(" · Page %d/%d", page, totalPages))
	}
	body.WriteString("\n\n")

	var buttons [][]ButtonOption
	var row []ButtonOption
	activeID := sessions.GetOrCreateActive(msg.SessionKey).GetAgentSessionID()
	for i := start; i < end; i++ {
		session := agentSessions[i]
		displayName := sessions.GetSessionName(session.ID)
		if displayName == "" {
			displayName = strings.Join(strings.Fields(strings.ReplaceAll(session.Summary, "\n", " ")), " ")
		}
		if displayName == "" {
		...
		body.WriteString(fmt.Sprintf("%s %d. %s · %d msgs · %s\n",
			marker, i+1, displayName, session.MessageCount, session.ModifiedAt.Format("01-02 15:04")))
		row = append(row, ButtonOption{
			Text: fmt.Sprintf("%s %d. %s", marker, i+1, displayName),
			Data: fmt.Sprintf("cmd:/resume pick %s %d", scope, i+1),
		})
		if len(row) == 1 {
	...
	if page > 1 {
		buttons = append(buttons, []ButtonOption{{Text: "← Previous", Data: fmt.Sprintf("cmd:/resume %s %d", scope, page-1)}})
	}
	if page < totalPages {
		buttons = append(buttons, []ButtonOption{{Text: "Next →", Data: fmt.Sprintf("cmd:/resume %s %d", scope, page+1)}})
	}
	buttons = append(buttons, []ButtonOption{{Text: "← Scopes", Data: "cmd:/resume"}})
	body.WriteString("\nTap a session to resume it.")
	e.replyWithButtons(p, msg.ReplyCtx, body.String(), buttons)
}
...
func (e *Engine) cmdName(p Platform, msg *Message, args []string) {
	...
	if setter, ok := agent.(SessionTitleSetter); ok {
		if err := setter.SetSessionTitle(e.ctx, targetID, name); err != nil {
			slog.Warn("failed to persist native session title", "session_id", targetID, "error", err)
		}
	}
...
func (e *Engine) cmdStatus(p Platform, msg *Message) {
	...
	if !strings.EqualFold(agent.Name(), "codex") {
		e.reply(p, msg.ReplyCtx, e.legacyStatusText(agent, sessions, msg))
		return
	}
	status := e.gapCodeStatusText(agent, sessions, msg)
	if supportsCards(p) {
		title, body := splitCardTitleBody(status)
		e.replyWithCard(p, msg.ReplyCtx, NewCard().
			Title(title, "green").
			Markdown(body).
...
func (e *Engine) legacyStatusText(agent Agent, sessions *SessionManager, msg *Message) string {
	...
	return e.i18n.Tf(MsgStatusTitle,
		e.name,
		agent.Name(),
		workDirStr,
		platformStr,
		...
		sessionStr,
		cronStr,
		e.i18n.Tf(MsgStatusSessionKey, msg.SessionKey),
		agentSIDStr,
		userIDStr,
	)
}

// gapCodeStatusText is the bridge's remote equivalent of GapCode's /status
// panel. It reads the active agent/session capabilities instead of reporting
// cc-connect transport metadata as if it were GapCode runtime state.
func (e *Engine) gapCodeStatusText(agent Agent, sessions *SessionManager, msg *Message) string {
	lang := e.i18n.CurrentLang()
	label := func(en, fa string) string {
		if lang == LangFarsi {
			return fa
		}
		return en
	...
	if state != nil {
		state.mu.Lock()
		running = state.agentSession
		state.mu.Unlock()
	}

	model := replyFooterModel(running, agent)
	effort := replyFooterReasoningEffort(running, agent)
	provider := "-"
	providerURL := ""
	if ps, ok := agent.(ProviderSwitcher); ok {
		if current := ps.GetActiveProvider(); current != nil {
			provider = current.Name
			providerURL = current.BaseURL
		}
	}
	if providerURL != "" {
	...
	permission := "-"
	if ms, ok := agent.(ModeSwitcher); ok {
		permission = gapCodePermissionLabel(ms.GetMode())
	}
	agentsFile := "-"
	if mp, ok := agent.(MemoryFileProvider); ok {
		path := mp.ProjectMemoryFile()
		if _, statErr := os.Stat(path); statErr == nil {
			agentsFile = filepath.Base(path)
		}
	}

	sessionID := active.GetAgentSessionID()
	if sessionID == "" && running != nil {
		sessionID = running.CurrentSessionID()
	}
	usage := contextUsageForStatus(running)
	tokenLine := "-"
	contextLine := "-"
	if usage != nil {
		...
		if total > 0 {
			tokenLine = formatStatusTokenCount(total) + " total"
		}
		if usage.ContextWindow > 0 {
			used := usage.UsedTokens
			if used <= 0 {
			...
			left := 100 - (used * 100 / usage.ContextWindow)
			if left < 0 {
				left = 0
			}
			contextLine = fmt.Sprintf("%d%% %s", left, label("left", "باقی‌مانده"))
		}
	}

	var quota string
	if report := e.statusUsage(agent, running); report != nil {
		quota = formatUsageBlocks(report, lang)
	}
	...
	b.WriteString("│                                                                              │\n")
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Model:", "مدل:"), value(model)))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Reasoning:", "استدلال:"), value(effort)))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Model provider:", "ارائه‌دهنده مدل:"), provider))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Directory:", "پوشه:"), value(e.commandWorkDir(agent, msg))))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Permissions:", "مجوزها:"), permission))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Agents.md:", "Agents.md:"), agentsFile))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Collaboration mode:", "حالت همکاری:"), label("Default", "پیش‌فرض")))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Session:", "نشست:"), value(sessionID)))
	b.WriteString("│                                                                              │\n")
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Token usage:", "مصرف توکن:"), tokenLine))
	b.WriteString(fmt.Sprintf("│  %-20s %s\n", label("Context window:", "پنجره زمینه:"), contextLine))
	if strings.TrimSpace(quota) != "" {
		for _, line := range strings.Split(strings.TrimSpace(quota), "\n") {
			b.WriteString(fmt.Sprintf("│  %-20s %s\n", "", line))
		}
	}
	b.WriteString("╰──────────────────────────────────────────────────────────────────────────────╯")
...
func gapCodePermissionLabel(mode string) string {
...
func contextUsageForStatus(session AgentSession) *ContextUsage {
	if session == nil {
		return nil
	}
...
func (e *Engine) statusUsage(agent Agent, session AgentSession) *UsageReport {
	var reporter UsageReporter
	if session != nil {
		reporter, _ = session.(UsageReporter)
	}
	if reporter == nil {
		reporter, _ = agent.(UsageReporter)
	}
	if reporter == nil {
		return nil
	}
	...
	if err != nil {
		slog.Debug("GapCode status usage unavailable", "error", err)
		return nil
	}
	return report
}
...
func (e *Engine) cmdUsage(p Platform, msg *Message) {
	...
	if state != nil {
		state.mu.Lock()
		active := state.agentSession
		state.mu.Unlock()
		if activeReporter, ok := active.(UsageReporter); ok {
...
func (e *Engine) cmdLang(p Platform, msg *Message, args []string) {
	...
	e.i18n.SetLang(lang)
	if _, _, err := e.setupMemoryFile(); err != nil {
		slog.Warn("failed to refresh localized agent instructions", "language", lang, "error", err)
	}
...
// Built-in command metadata is always English; runtime replies remain localized.
func (e *Engine) GetAllCommands() []BotCommandInfo {
	...
	filtered := make([]BotCommandInfo, 0, len(commands))
	for _, command := range commands {
		if !disabledCmds[command.Command] {
...
func (e *Engine) cmdModel(p Platform, msg *Message, args []string) {
	...
	// GapCode's model picker continues directly into reasoning effort
	// selection. Keep that interaction intact for Telegram/Bale inline
	// keyboards instead of ending after the model is changed.
	if _, ok := p.(InlineButtonSender); ok {
