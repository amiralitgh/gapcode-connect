...
func TestMenuCommandsForPlatform_TelegramNeverPublishesSkills(t *testing.T) {
	...
	if skillsOmitted {
		t.Fatalf("skillsOmitted = true, want false because skills are excluded before menu planning")
	}
...
func TestCmdSkills_TelegramDoesNotClaimMenuOverflow(t *testing.T) {
	p := &stubPlatformEngine{n: "telegram"}
	e := NewEngine("test", &stubAgent{}, []Platform{p}, "", LangEnglish)
	temp := t.TempDir()
	for i := 0; i < 100; i++ {
	...
	if strings.Contains(p.sent[0], "command menu is full") {
		t.Fatalf("skills text = %q, should not claim menu overflow", p.sent[0])
	}
}
...
func TestExecuteCardAction_LangSwitch(t *testing.T) {
	...
	e.executeCardAction("/lang", "fa", "test:user1")
	if e.i18n.CurrentLang() != LangFarsi {
		t.Errorf("expected LangFarsi, got %v", e.i18n.CurrentLang())
	}
	...
	if e.i18n.CurrentLang() != LangEnglish {
		t.Errorf("unsupported language changed locale to %v", e.i18n.CurrentLang())
	}
}
