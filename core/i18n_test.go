...
func TestDetectLanguage(t *testing.T) {
	tests := []struct {
		...
		{"سلام، وضعیت پروژه چطور است؟", LangFarsi},
		// English (default)
...
func TestNormalizeLanguageStringFarsi(t *testing.T) {
	if got := NormalizeLanguageString("farsi"); got != LangFarsi {
		t.Fatalf("NormalizeLanguageString(farsi) = %q, want %q", got, LangFarsi)
	}
}

func TestSupportedBotLanguagesAreEnglishAndFarsi(t *testing.T) {
	for _, raw := range []string{"en", "english", "fa", "farsi", "فارسی", "auto", ""} {
		if got := NormalizeLanguageString(raw); got != LangEnglish && got != LangFarsi && got != LangAuto {
			t.Errorf("NormalizeLanguageString(%q) = %q, want English, Farsi, or Auto", raw, got)
		}
	}
	for _, raw := range []string{"zh", "zh-TW", "ja", "es", "chinese", "japanese", "spanish"} {
		if got := NormalizeLanguageString(raw); got != LangAuto {
			t.Errorf("NormalizeLanguageString(%q) = %q, want unsupported languages to resolve to Auto", raw, got)
		}
	}
}

func TestI18nSetLangPersistsExplicitLanguage(t *testing.T) {
	var saved Language
	i := NewI18n(LangEnglish)
	...
	i.SetLang(LangFarsi)
	if saved != LangFarsi {
		t.Fatalf("SetLang saved %q, want %q", saved, LangFarsi)
	}
}

func TestFarsiUsesLocalizedBridgeMessages(t *testing.T) {
	got := NewI18n(LangFarsi).T(MsgStarting)
	if got == "⏳ Processing..." || got == "" {
		t.Fatalf("Farsi MsgStarting = %q, want a Persian message", got)
	}
}

func TestFarsiTranslationOrFallback(t *testing.T) {
	if got := NewI18n(LangFarsi).T(MsgStarting); got == "" {
		t.Fatal("Farsi translation/fallback was empty")
	}
}

func TestPublicBrandingUsesGapCodeConnect(t *testing.T) {
	for _, lang := range []Language{LangEnglish, LangFarsi} {
		i := NewI18n(lang)
		for _, key := range []MsgKey{MsgWelcome, MsgHelpTitle, MsgStatusTitle, MsgCardTitleStatus, MsgRestarting, MsgRestartSuccess, MsgBuiltinCmdVersion} {
			got := i.T(key)
			if strings.Contains(strings.ToLower(got), "cc-connect") {
				t.Fatalf("%s %s = %q, contains legacy public brand", lang, key, got)
			}
		}
	}
}
