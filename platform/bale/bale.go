...
var baleConvertAudioToOpus = core.ConvertAudioToOpus

func init() {
	core.RegisterPlatform("bale", New)
}
...
// baleBot abstracts the Bale bot API methods for testability.
// *tgbot.Bot satisfies this interface.
type baleBot interface {
...
type botFactory func(token string, onUpdate func(context.Context, *models.Update), httpClient *http.Client) (baleBot, *models.User, func(context.Context), error)
...
type Platform struct {
	...
	progressStyle         string // "legacy" | "compact" — bale has no rich card, so "card" is mapped to "compact"
...
func New(opts map[string]any) (core.Platform, error) {
	return newPlatform(opts, "bale", "https://tapi.bale.ai")
}

// NewBale creates a Bale-compatible Bot API platform against Bale's
// native endpoint. Bale uses the same update/message shapes, while its API
// host is different.
func NewBale(opts map[string]any) (core.Platform, error) {
	return newPlatform(opts, "bale", "https://tapi.bale.ai")
}

func newPlatform(opts map[string]any, platformName, defaultAPIBaseURL string) (core.Platform, error) {
	...
	if v, ok := opts["api_base_url"].(string); ok && strings.TrimSpace(v) != "" {
		validatedURL, err := validateAPIBaseURL(strings.TrimSpace(v))
		if err != nil {
			return nil, fmt.Errorf("%s: invalid api_base_url: %w", platformName, err)
		}
	...
	// to avoid the HTTP client racing with Bale's response. 90s gives 30s headroom.
	httpClient := &http.Client{Timeout: 90 * time.Second}
	if proxyURL, _ := opts["proxy"].(string); proxyURL != "" {
		u, err := url.Parse(proxyURL)
		if err != nil {
			return nil, fmt.Errorf("%s: invalid proxy URL %q: %w", platformName, proxyURL, err)
		}
		...
		slog.Info(platformName+": using proxy", "proxy", u.Host, "auth", proxyUser != "")
	}
	...
	// Default to "compact" so streaming edits work out of the box. Bale has
	...
	if v, ok := opts["progress_style"].(string); ok {
		...
		case "compact", "card":
			progressStyle = "compact"
		default:
			return nil, fmt.Errorf("%s: invalid progress_style %q (want legacy, compact, or card)", platformName, v)
		}
	}
	...
	return &Platform{
		...
		newBot: func(token string, onUpdate func(context.Context, *models.Update), httpClient *http.Client) (baleBot, *models.User, func(context.Context), error) {
			return defaultNewBotAtURL(token, onUpdate, httpClient, apiBaseURL)
		},
	}, nil
}
...
func validateAPIBaseURL(raw string) (string, error) {
	...
	return "", fmt.Errorf("HTTPS is required except for localhost test servers")
}

func (p *Platform) Name() string {
	if p == nil || p.platformName == "" {
		return "bale"
	}
...
//
// Bale supports MessageUpdater.UpdateMessage, so we default to "compact"
...
func (p *Platform) Start(handler core.MessageHandler) error {
	...
	if p.stopping {
		return fmt.Errorf("bale: platform stopped")
	}
	if p.newBot == nil {
...
func defaultNewBot(token string, onUpdate func(context.Context, *models.Update), httpClient *http.Client) (baleBot, *models.User, func(context.Context), error) {
	return defaultNewBotAtURL(token, onUpdate, httpClient, "https://tapi.bale.ai")
}

func defaultNewBotAtURL(token string, onUpdate func(context.Context, *models.Update), httpClient *http.Client, apiBaseURL string) (baleBot, *models.User, func(context.Context), error) {
	...
	opts := []tgbot.Option{
		...
		tgbot.WithServerURL(apiBaseURL),
	}
	if httpClient != nil {
...
func (p *Platform) runConnection(ctx context.Context) error {
	...
	if err != nil {
		...
		return &retryLoopError{
			cause: cause,
			err:   fmt.Errorf("bale: connect failed: %w", err),
		}
	}
	if ctx.Err() != nil || p.isStopping() {
		return nil
	...
	slog.Info("bale: connected", "bot", me.Username)
	p.emitReady(gen)
...
func (p *Platform) handleMessage(ctx context.Context, msg *models.Message) {
	...
	if core.IsOldMessage(msgTime) {
		slog.Debug("bale: ignoring old message after restart", "date", msgTime)
		return
	}
	if msg.From == nil {
		return
	...
	if !core.AllowList(p.allowFrom, userID) {
		// Keep identity discovery available during first-time Bale setup. Bale
		// user IDs are platform-specific, so copying a Telegram allow_from ID
		// can otherwise make the bot appear completely silent.
		if !isIdentityDiscoveryCommand(msg.Text) {
			slog.Warn("bale: message rejected by allow_from", "user_id", userID)
			return
		}
		slog.Info("bale: allowing identity discovery for unlisted user", "user_id", userID)
	}
	...
	if isGroup && !p.groupReplyAll {
		slog.Debug("bale: checking group message", "text", msg.Text, "is_command", isCommand(msg))
	...
	if len(msg.Photo) > 0 {
		...
		if err != nil {
			slog.Error("bale: download photo failed", "error", err)
			return
		}
		caption := stripBotMention(msg.Caption, botName)
		p.dispatchMessage(&core.Message{
			SessionKey: sessionKey, Platform: "bale",
	...
	if msg.Voice != nil {
		slog.Debug("bale: voice received", "user", userName, "duration", msg.Voice.Duration)
		audioData, err := p.downloadFile(msg.Voice.FileID)
		if err != nil {
			slog.Error("bale: download voice failed", "error", err)
			return
		}
		p.dispatchMessage(&core.Message{
			SessionKey: sessionKey, Platform: "bale",
	...
	if msg.Audio != nil {
		slog.Debug("bale: audio file received", "user", userName)
		audioData, err := p.downloadFile(msg.Audio.FileID)
		if err != nil {
			slog.Error("bale: download audio failed", "error", err)
			return
		}
		format := "mp3"
	...
	if msg.Document != nil {
		slog.Info("bale: document received", "user", userName, "file_name", msg.Document.FileName, "mime", msg.Document.MimeType, "file_id", msg.Document.FileID)
		fileData, err := p.downloadFile(msg.Document.FileID)
		if err != nil {
			slog.Error("bale: download document failed", "error", err)
			return
		}
		caption := stripBotMention(msg.Caption, botName)
		p.dispatchMessage(&core.Message{
	...
	if msg.Location != nil {
		slog.Info("bale: location received", "user", userName, "latitude", msg.Location.Latitude, "longitude", msg.Location.Longitude)
		p.dispatchMessage(&core.Message{
			SessionKey: sessionKey, Platform: "bale",
			UserID: userID, UserName: userName, ChatName: chatName,
			MessageID:  strconv.Itoa(msg.ID),
	...
	slog.Debug("bale: message received", "user", userName, "chat", msg.Chat.ID)
	p.dispatchMessage(&core.Message{
		SessionKey: sessionKey, Platform: "bale",
		UserID: userID, UserName: userName, ChatName: chatName,
		Content:    text,
...
func (p *Platform) dispatchMessage(msg *core.Message, tgMsg *models.Message) {
	...
	// receives the URL. Bale drops the href when forwarding plain text (#1207).
...
// reactToMessage sets an emoji reaction on a Bale message.
...
func (p *Platform) reactToMessage(ctx context.Context, chatID int64, messageID int, emoji string) {
	...
	if _, err := bot.SetMessageReaction(ctx, &tgbot.SetMessageReactionParams{
		...
		slog.Debug("bale: set reaction failed", "error", err)
	}
}
...
func (p *Platform) buildSessionKey(chatID int64, threadID int, userID int64) string {
	if p.shareSessionInChannel {
		if threadID != 0 {
			return fmt.Sprintf("bale:%d:%d", chatID, threadID)
		}
		return fmt.Sprintf("bale:%d", chatID)
	}
	if threadID != 0 {
		return fmt.Sprintf("bale:%d:%d:%d", chatID, threadID, userID)
	}
	return fmt.Sprintf("bale:%d:%d", chatID, userID)
}
...
func (p *Platform) publishBot(b baleBot, me *models.User) (uint64, bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *Platform) clearBot(gen uint64, b baleBot) {
	notify := false
	p.mu.Lock()
	...
	if notify {
		p.notifyUnavailable(fmt.Errorf("bale: connection lost"))
	}
}

func (p *Platform) connectedBot(action string) (baleBot, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	if p.bot == nil {
		return nil, fmt.Errorf("bale: %s: bot not connected", action)
	}
	return p.bot, nil
}
...
func retryLogMessage(cause retryCause) string {
	...
	case retryCauseInitialConnectFailure:
		return "bale: initial connection failed, retrying"
	case retryCauseConnectionLost:
		return "bale: connection lost, retrying"
	default:
		return "bale: reconnect failed, retrying"
	}
}
...
func (p *Platform) handleCallbackQuery(ctx context.Context, cb *models.CallbackQuery) {
	...
	if err != nil {
		slog.Debug("bale: ignoring callback for disconnected bot", "error", err)
		return
	}
	...
	if !core.AllowList(p.allowFrom, userID) {
		slog.Debug("bale: callback from unauthorized user", "user", userID)
		return
	}
	...
	if _, err := bot.AnswerCallbackQuery(ctx, &tgbot.AnswerCallbackQueryParams{CallbackQueryID: cb.ID}); err != nil {
		slog.Debug("bale: answer callback failed", "error", err)
	}
	...
	if strings.HasPrefix(data, "cmd:") {
		...
		if _, err := bot.EditMessageText(ctx, &tgbot.EditMessageTextParams{
			...
			slog.Debug("bale: callback edit failed", "error", err)
		}
	...
	default:
		slog.Debug("bale: unknown callback data", "data", data)
		return
	}
	...
	if _, err := bot.EditMessageText(ctx, &tgbot.EditMessageTextParams{
		...
		slog.Debug("bale: permission callback edit failed", "error", err)
	}
...
func (p *Platform) isDirectedAtBot(msg *models.Message) bool {
	...
	p.mu.RUnlock()
	if self == nil {
		slog.Debug("bale: ignoring group routing, self user unknown")
		return false
	}
	...
	if isCommand(msg) {
		...
		if atIdx > 0 && atIdx < cmdEnd {
			...
			slog.Debug("bale: command with @suffix", "bot", botName, "target", target, "match", strings.EqualFold(target, botName))
			return strings.EqualFold(target, botName)
		}
		slog.Debug("bale: command without @suffix, accepting", "bot", botName, "text", msg.Text)
	...
	if msg.Entities != nil {
		for _, e := range msg.Entities {
			if e.Type == models.MessageEntityTypeMention {
				...
				slog.Debug("bale: checking mention", "bot", botName, "mention", mention, "match", strings.EqualFold(mention, "@"+botName))
	...
	if msg.ReplyToMessage != nil && msg.ReplyToMessage.From != nil {
		slog.Debug("bale: checking reply", "bot_id", self.ID, "reply_from_id", msg.ReplyToMessage.From.ID)
	...
	slog.Debug("bale: ignoring group message not directed at bot", "chat", msg.Chat.ID, "bot", botName, "text", msg.Text, "entities", msg.Entities)
	return false
}
...
// isIdentityDiscoveryCommand recognizes only the exact /whoami command.
// It is intentionally narrow because this is the sole command allowed to
// cross the allow_from boundary during initial platform setup.
func isIdentityDiscoveryCommand(text string) bool {
...
func (p *Platform) Reply(ctx context.Context, rctx any, content string) error {
	...
	if !ok {
		return fmt.Errorf("bale: invalid reply context type %T", rctx)
	}
	...
	if _, err := bot.SendMessage(ctx, params); err != nil {
		...
		if strings.Contains(errMsg, "can't parse") {
			slog.Warn("bale: HTML rejected by Bale, sending as plain text",
				"method", "Reply",
				"error", errMsg,
			...
			slog.Warn("bale: message too long, splitting into chunks",
				"method", "Reply",
				"html_len", len(html),
			)
		...
		if err != nil {
			return fmt.Errorf("bale: send: %w", err)
		}
	}
	return nil
}
...
func (p *Platform) SendImage(ctx context.Context, rctx any, img core.ImageAttachment) error {
	...
	slog.Debug("bale: sending image", "chat_id", rc.chatID, "name", name, "size", len(img.Data))
	...
	if _, err := bot.SendPhoto(ctx, params); err != nil {
		return fmt.Errorf("bale: send image: %w", err)
	}
	return nil
}
...
func (p *Platform) SendFile(ctx context.Context, rctx any, file core.FileAttachment) error {
	...
	if _, err := bot.SendDocument(ctx, params); err != nil {
		return fmt.Errorf("bale: send file: %w", err)
	}
	return nil
}

// SendAudio sends synthesized audio back to Bale.
...
func (p *Platform) SendAudio(ctx context.Context, rctx any, audio []byte, format string) error {
	rc, ok := rctx.(replyContext)
	if !ok {
		return fmt.Errorf("bale: SendAudio: invalid reply context type %T", rctx)
	}
	...
	default:
		converted, err := baleConvertAudioToOpus(ctx, audio, sendFormat)
		if err != nil {
			return fmt.Errorf("bale: SendAudio: convert %s to opus: %w", sendFormat, err)
		}
		sendData = converted
		sendFormat = "opus"
	}
	...
	if err := p.sendVoice(ctx, rc, sendData, sendFormat); err != nil {
		...
		return fmt.Errorf("bale: SendAudio: sendVoice: %w", err)
	}
	return nil
}
...
func (p *Platform) sendVoice(ctx context.Context, rc replyContext, audio []byte, format string) error {
	...
	params := &tgbot.SendVoiceParams{
		ChatID:          rc.chatID,
		MessageThreadID: rc.threadID,
		Voice:           &models.InputFileUpload{Filename: "tts_audio." + baleAudioFileExt(format), Data: bytes.NewReader(audio)},
	}
...
func (p *Platform) sendAudio(ctx context.Context, rc replyContext, audio []byte, format string) error {
	...
	params := &tgbot.SendAudioParams{
		ChatID:          rc.chatID,
		MessageThreadID: rc.threadID,
		Audio:           &models.InputFileUpload{Filename: "tts_audio." + baleAudioFileExt(format), Data: bytes.NewReader(audio)},
	}
...
func baleAudioFileExt(format string) string {
...
func (p *Platform) SendWithButtons(ctx context.Context, rctx any, content string, buttons [][]core.ButtonOption) error {
	...
	if _, err := bot.SendMessage(ctx, params); err != nil {
		...
		if err != nil {
			return fmt.Errorf("bale: sendWithButtons: %w", err)
		}
	}
	return nil
}
...
func (p *Platform) DeletePreviewMessage(ctx context.Context, previewHandle any) error {
	h, ok := previewHandle.(*balePreviewHandle)
	if !ok {
		return fmt.Errorf("bale: invalid preview handle type %T", previewHandle)
	}
	...
	if err != nil {
		slog.Debug("bale: delete preview message failed", "error", err)
	}
	return err
}
...
func (p *Platform) ReconstructReplyCtx(sessionKey string) (any, error) {
	// Formats:
	//   bale:{chatID}                      - shared session, no topic
	//   bale:{chatID}:{threadID}           - shared session, with topic
	//   bale:{chatID}:{userID}             - per-user session, no topic
	//   bale:{chatID}:{threadID}:{userID}  - per-user session, with topic
	parts := strings.SplitN(sessionKey, ":", 5)
	if len(parts) < 2 || parts[0] != "bale" {
		return nil, fmt.Errorf("bale: invalid session key %q", sessionKey)
	}
	chatID, err := strconv.ParseInt(parts[1], 10, 64)
	if err != nil {
		return nil, fmt.Errorf("bale: invalid chat ID in %q", sessionKey)
	}
	...
	case 3:
		if p.shareSessionInChannel {
			// bale:{chatID}:{threadID}
			threadID, err = strconv.Atoi(parts[2])
			if err != nil {
				slog.Warn("bale: invalid thread ID", "raw", parts[2], "error", err)
			}
		}
		// else: bale:{chatID}:{userID} — no threadID
	case 4:
		// bale:{chatID}:{threadID}:{userID}
		threadID, err = strconv.Atoi(parts[2])
		if err != nil {
			slog.Warn("bale: invalid thread ID", "raw", parts[2], "error", err)
		}
...
// balePreviewHandle stores the chat, thread, and message IDs for an editable preview message.
type balePreviewHandle struct {
	chatID    int64
	threadID  int
	messageID int
}
...
func (p *Platform) SendPreviewStart(ctx context.Context, rctx any, content string) (any, error) {
	rc, ok := rctx.(replyContext)
	if !ok {
		return nil, fmt.Errorf("bale: invalid reply context type %T", rctx)
	}
	...
	if err != nil {
		...
		// Handle HTML parsing errors by falling back to plain text
		if strings.Contains(errMsg, "can't parse") {
			slog.Warn("bale: HTML rejected by Bale, sending preview as plain text",
			...
			slog.Warn("bale: preview too long, sending as plain text",
				"method", "SendPreviewStart",
				"html_len", len(html),
		...
		if err != nil {
			return nil, fmt.Errorf("bale: send preview: %w", err)
		}
	}
	return &balePreviewHandle{chatID: rc.chatID, threadID: rc.threadID, messageID: sent.ID}, nil
}
...
func (p *Platform) UpdateMessage(ctx context.Context, previewHandle any, content string) error {
	...
	if _, err := bot.EditMessageText(ctx, params); err != nil {
		errMsg := err.Error()
		slog.Debug("bale: UpdateMessage HTML failed", "error", errMsg)
		if strings.Contains(errMsg, "not modified") {
			return nil
		}
		if strings.Contains(errMsg, "can't parse") {
			slog.Warn("bale: HTML rejected by Bale, editing as plain text",
			...
			if _, err2 := bot.EditMessageText(ctx, params); err2 != nil {
				...
				return fmt.Errorf("bale: edit message: %w", err2)
			}
			return nil
		}
		return fmt.Errorf("bale: edit message: %w", err)
	}
	slog.Debug("bale: UpdateMessage HTML success")
	return nil
}

// baleMaxMessageLen is the maximum message length for Bale.
// Bale's limit is 4096 characters for text messages.
const baleMaxMessageLen = 4096
...
func (p *Platform) sendChunked(ctx context.Context, bot baleBot, rc replyContext, html string) error {
	chunks := core.SplitMessageCodeFenceAware(html, baleMaxMessageLen)
		...
		if _, err := bot.SendMessage(ctx, params); err != nil {
			...
			if strings.Contains(err.Error(), "can't parse") {
				...
				if _, err2 := bot.SendMessage(ctx, params); err2 != nil {
					return fmt.Errorf("bale: send chunk %d: %w", i, err2)
				}
			} else {
				return fmt.Errorf("bale: send chunk %d: %w", i, err)
			}
		}
	}
	return nil
...
func (p *Platform) sendChunkedWithButtons(ctx context.Context, bot baleBot, rc replyContext, html string, rows [][]models.InlineKeyboardButton) error {
	chunks := core.SplitMessageCodeFenceAware(html, baleMaxMessageLen)
	for i, chunk := range chunks {
		params := &tgbot.SendMessageParams{
			ChatID:          rc.chatID,
...
func (p *Platform) StartTyping(ctx context.Context, rctx any) (stop func()) {
	...
	if bot, err := p.connectedBot("typing"); err == nil {
		if _, err := bot.SendChatAction(ctx, params); err != nil {
			slog.Debug("bale: initial typing send failed", "error", err)
		}
	} else {
		return func() {}
	}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C():
				...
				if err != nil {
					slog.Debug("bale: typing stopped", "error", err)
					return
				}
				if _, err := bot.SendChatAction(ctx, params); err != nil {
					slog.Debug("bale: typing send failed", "error", err)
				}
			}
		}
	}()
...
const baleBotCommandDescriptionLimit = 40

// truncateBaleBotDescription keeps Bale command descriptions within a
// conservative safety budget. Bale documents a larger per-field limit, but
...
func truncateBaleBotDescription(s string) string {
	const max = baleBotCommandDescriptionLimit
...
// RegisterCommands registers bot commands with Bale for the command menu.
func (p *Platform) RegisterCommands(commands []core.BotCommandInfo) error {
	...
	// Bale limits: max 100 commands; keep descriptions conservatively short
	// to avoid menu registration failures with larger command sets.
	var tgCommands []models.BotCommand
	seen := make(map[string]bool)
	for _, c := range commands {
		cmd := sanitizeBaleCommand(c.Command)
		if cmd == "" || seen[cmd] {
			continue
		}
		seen[cmd] = true
		desc := truncateBaleBotDescription(c.Description)
	...
	if len(tgCommands) == 0 {
		slog.Debug("bale: no commands to register")
		return nil
	}

	ctx := context.Background()
	if _, err := bot.SetMyCommands(ctx, &tgbot.SetMyCommandsParams{Commands: tgCommands}); err != nil {
		if isBaleCommandMenuUnsupported(err) {
			slog.Warn("bale: command menu is not supported by this API; slash commands remain available",
				"error", err)
			return nil
		}
		return fmt.Errorf("bale: setMyCommands failed: %w", err)
	}

	slog.Info("bale: registered bot commands", "count", len(tgCommands))
	return nil
}

func isBaleCommandMenuUnsupported(err error) bool {
	if err == nil {
		return false
	}
	message := strings.ToLower(err.Error())
	return strings.Contains(message, "501") &&
		(strings.Contains(message, "not implemented") || strings.Contains(message, "coming soon"))
}

// extractEntityText extracts a substring from text using Bale's UTF-16 code unit
// offset and length. Bale Bot API entity offsets are measured in UTF-16 code units,
...
func enrichTextLinks(text string, entities []models.MessageEntity) string {
	...
	// Work in UTF-16 code units (Bale's coordinate system).
	runes := []rune(text)
...
// sanitizeBaleCommand converts a command name to Bale-compatible format.
// Bale rules: 1-32 chars, lowercase letters/digits/underscores, must start with a letter.
// Returns "" if the command cannot be sanitized (e.g. empty or no letter to start with).
func sanitizeBaleCommand(cmd string) string {
