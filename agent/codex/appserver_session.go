...
func newAppServerSession(ctx context.Context, url, workDir, cmd, model, effort, mode, resumeID, baseURL, modelProvider string, extraEnv []string, codexHome string, systemPrompt string, appendPrompt string) (*appServerSession, error) {
	sessionCtx, cancel := context.WithCancel(ctx)
	s := &appServerSession{
		url:              url,
		workDir:          workDir,
		cliBin:           strings.TrimSpace(cmd),
		model:            model,
		effort:           effort,
		mode:             mode,
		baseURL:          baseURL,
...
func (s *appServerSession) connect() error {
	...
	cmd := exec.CommandContext(s.ctx, cliBin, args...)
	cmd.Dir = s.workDir
