...
import (
	...
	"github.com/amiralitgh/gapcode-connect/agent/internal/skillroots"
...
func New(opts map[string]any) (core.Agent, error) {
	...
	defaultCmd := "gapcode"
	if _, err := exec.LookPath(defaultCmd); err != nil {
		defaultCmd = "codex"
	}
	cmd, cliExtraArgs := core.ParseCmdOpts(opts, defaultCmd)

	if _, err := exec.LookPath(cmd); err != nil {
		return nil, fmt.Errorf("codex: %q CLI not found in PATH, install GapCode or OpenAI Codex CLI", cmd)
	}

	runtimeModel, runtimeEffort := readCodexRuntimeDefaults(codexHome)
	if strings.TrimSpace(model) == "" {
		model = runtimeModel
	}
	if strings.TrimSpace(reasoningEffort) == "" {
		reasoningEffort = runtimeEffort
	}
...
func (a *Agent) AvailableReasoningEfforts() []string {
	current := a.GetModel()
	if efforts := readCodexReasoningEfforts(current); len(efforts) > 0 {
		return efforts
	}
...
func parseCodexModelsJSON(data []byte) []core.ModelOption {
	...
	for _, m := range payload.Models {
		...
		seen[name] = struct{}{}
		models = append(models, core.ModelOption{Name: name, Desc: strings.TrimSpace(m.Description)})
	}
	return models
}

func readCodexReasoningEfforts(modelName string) []string {
	...
	return parseCodexReasoningEffortsJSON(data, modelName)
}

func parseCodexReasoningEffortsJSON(data []byte, modelName string) []string {
	var payload struct {
		Models []struct {
			Slug                     string `json:"slug"`
			DisplayName              string `json:"display_name"`
...
func readCodexRuntimeDefaults(codexHome string) (string, string) {
	...
	return strings.TrimSpace(cfg.Model), normalizeReasoningEffort(cfg.ModelReasoningEffort)
}
...
func (a *Agent) StartSession(ctx context.Context, sessionID string) (core.AgentSession, error) {
	...
	if sessionID != "" && sessionID != core.ContinueSession &&
		threadWriterLockHeld(resolveCodexHomeDir(codexHome), sessionID) {
		return nil, sessionWriterLockError(sessionID)
	}
	...
	if backend == "app_server" {
		return newAppServerSession(ctx, appServerURL, workDir, cliBin, model, reasoningEffort, mode, sessionID, baseURL, provName, extraEnv, codexHome, systemPrompt, appendPrompt)
	}
	if codexHome != "" {
...
func (a *Agent) ListAllSessions(_ context.Context) ([]core.AgentSessionInfo, error) {
	...
	codexHome := a.codexHome
	a.mu.RUnlock()
	return listCodexSessionsAll(codexHome)
}
...
func (a *Agent) SetSessionTitle(_ context.Context, sessionID, title string) error {
	...
	codexHome = resolveCodexHomeDir(codexHome)
	if codexHome == "" {
		return fmt.Errorf("resolve Codex state directory")
	}
...
// See: https://github.com/amiralitgh/gapcode-connect/issues/378
