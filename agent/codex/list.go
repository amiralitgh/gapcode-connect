...
// resolveCodexHomeDir returns the effective Codex state directory.
// GapCode is a bundled Codex distribution whose state lives under ~/.gapcode;
// keep explicit/upstream overrides first, then discover that state root before
// falling back to the upstream ~/.codex location.
func resolveCodexHomeDir(explicit string) string {
	...
	gapcodeHome := filepath.Join(homeDir, ".gapcode")
	if info, err := os.Stat(gapcodeHome); err == nil && info.IsDir() {
		return gapcodeHome
	}
...
func listCodexSessions(workDir, codexHome string) ([]core.AgentSessionInfo, error) {
	return listCodexSessionsFiltered(workDir, codexHome, true)
}

func listCodexSessionsAll(codexHome string) ([]core.AgentSessionInfo, error) {
	return listCodexSessionsFiltered("", codexHome, false)
}

func listCodexSessionsFiltered(workDir, codexHome string, filterCwd bool) ([]core.AgentSessionInfo, error) {
	...
	for _, f := range files {
		...
		info := parseCodexSessionFile(f, filter)
		if info != nil {
